...
using System.Text;
#region Değişkenler
// var yil = 2023; // Derleme aşamasında veri tipini kendisi belirliyor

// Type safety, tip güvenli
//Bellekte kaplayacagı alanı onceden ayırmıs oluyoruz hem de, alabileceği değerleri ve aralığını belirlemiş oluyoruz 

// İlkel/Değer tip 
// int yil = 2023; //32-bit  ~~2.1mr - ~2.1mr // default değeri : 0 (ondalık) // 00000000 (ikilik)
// short yil = 2023; //16-bit ~~32bin - ~32bin // default : 0


ushort yil = 2023; //unsigned // 16-bit 0 - ~65bin // default : 0

byte yas = 25; // 0-255 arası sayılar
long hesaplamaTamSayiSonucu = 9999999999999; // 64-bit // default : 0

//float dolarKuru = 20.06f; // 32-bit // default : 0
double dolarKuru = 29.062354f; //64-bit // default : 0
decimal hesaplamaSonucu = 29.0751485383482345m; // 128-bit  //genelde para ve hesaplama konusunda kullanılır.Hassas verilerde. // default : 0

bool dogruMu = true; // 1-bit // default : false // 00000000 (sinyal gelmiyorsa 8tane sıfır gelir)

char karakter = 'A'; // 65 //Ek bilgi: Unicode  ASCII tablolarına //16-bit // default: 0 //Unicode  ASCII U+0000 (NULL) 

// String aslında bir referans tip, İlkel/Değer tip
string isim = "Bilçağ"; // 32-bit // 16mr-bit'e kadar genişleyebiliyor // default : null

//string? takmaAd = null;
//if (takmaAd != null)
//    Console.WriteLine(takmaAd.ToUpper()); //takmaAd null ise ToUpper çalıştırmaya çalışmayacak ve null değerini bize verecek
//Console.WriteLine(takmaAd?.ToUpper()); //takmaAd null ise ToUpper çalıştırmaya çalışmayacak ve null değerini bize verecek
//Console.WriteLine(takmaAd ?? "Takma isim buılunmuyor"); //takmaAd null ise sağdaki değeri bize verecek.

// 0 (ondalık değerde defalut varsayılan deger sıfır) // 00000000 (ikilik)

// EK BİLGİ : DRY - Do Not Repeat Yourself
// EK BİLGİ : mccall software quality model // ilk yazılımsal makale yazılımın kalıtesını belırleyen cok onceden belırlenen terımlerı ınceler 
...
//int tamSayi = 10;
//double ondalikSayi = tamSayi; // 10.0 //

//double ondalikSayi2 = 10.5;
////int tamSayi2 = ondalikSayi2; // Veri kaybı olacagı ıcın ımplıst sekilde tür dönüşümü yapmayacaktır.
//int tamSayi2 = (int)ondalikSayi2; //Explist (bilinçli) tür dönüşümü //Casting

//long buyukTamSayi = long.MaxValue; // 64-bit
//int normalTamSayi = (int)buyukTamSayi; // 64-bit -> 32-bit // hata ; -1

////Daha güvenli tür donusumlerı ıcın :
////int normalTamSayi2 = Convert.ToInt32(buyukTamSayi); // Error : değer int'e gore cok buyuk
//string metinselTamsayi = buyukTamSayi.ToString();

////bool 2OndalikSayi // Invalid
//double ondalik_sayi = long.MaxValue; // Valid
////double doble = double.MaxValue; // Invalid
//double @double = double.MaxValue; //valid
...
//Console.WriteLine(cokBuyukTamsayi + 1); //Sayımız ilgili veri tipinin MinValue değerine geri doner.
#endregion
...
//mesaj = komut == "add" ? "Ekleme işlemi tamamlandı" :
...
//    index < 10; // Her döngünün başında kontrol ettigimiz koşul
//    index++)  // Her döngünün sonunda çalışan atama komutumuz

//{
//    if (index == 5)
//        continue;  // Bu döngü adımına devam etmeden bir sonraki dongu adımına geçer
...
//while (index2 < 10)
//{
//    //if (index2 == 5) continue; //Sonsuz döngü olusturmus oluyoruz
//    if (index2 == 5)
//    {
//        index2 += 1;
//        continue;
...
//while( true ) // Sonsuz döngülerden her daim kacınmamız gerekıyor, cunku program o noktada tıkanacaktır
//{
//    //istisna durumlarda sürekli kontrol için sonsuz donguler olusturulabılır fakat programı aksatmayacak ayrı bir işlem parcacıgında
//}


//do
//{
//    // Dömgünün ilk adımında herhangi bir koşula tabi olmadan ilgili işlemler yapılır
//    // Döngünün sonraki adımlarında
//} while (false); // Her Döngünün sonunda kontrol ettigimiz koşul
...
//    Console.WriteLine(icerik);  // Printer'a içeriği yazdıran fonskiyon olarak da düşünebiliriz.
//}
...
////ref , out // temel/ilkel veri tipleri üzerinde kullanılkır

//// ref, fonksiyona kendı ıcınde degısken olusturma, referans ettıgım degıskenı kullan demıs oluyoruz. 
//void IkiyleCarpımınıDosyaOlarakKaydet(ref int sayi, out int sayi2)
//{
//    sayi *= 2;
//    sayi2 = 1; // out, Fonksiyon/metot içinde ilk atama işlemini verme zorunlulugu getirir.

//    Console.WriteLine($"ikiyleCarpımınıDosyaOlarakKaydet scope sayi : {sayi}");
//    //.. bu parametre değişkenini kullanarak baska ıslemler yaptıgını varsayalım
//}
...
//Console.WriteLine("ana scope sayi " + sayi + " sayiOunt: " + sayiOut);
...
//Console.WriteLine("------------ Diziler ve Koleksiyonlar ------------");
//// Referans Tip
//string[] sira = new string[5]; // 5 dizinin içerisindeki yer sayısı
//sira[0] = "Muhammet";
//sira[2] = "Umut";
...
//{
//    string sıradakiKişininAdı = sira[index];

//  Console.WriteLine($"sıra dizisindeki {index}. indexteki değeri : {sıradakiKişininAdı}");
//}
//foreach (string sıradakiKişininAdı in sıra)
//{
//    Console.WriteLine(sıradakiKişininAdı);
//}

//string[] sira2 = { "Muhammet", "Umut" }; //new string[2] // Verdigimiz başlangıç değeri kadar array olusturacaktır.
...
// Dictionary
// Anahtar ve Değer eşleşmesiyle bir sözlük yapısı olusturur.
//Dictionary<string, string> keyValuePairs = new Dictionary<string, string>();
//Dictionary<string, string> sira5 = new();

//sira5.Add(key: "Birinci Sıra", "Ahmet");
...
//// HashSet 
//// İçindeki elemanların tamamen birbirinden farklı olması gerekir. 
...
//Class'lar Nesneler olusturmak ıcın bir şablondur diyebiliriz.
